...
class Detector(object):
    def __init__(self,net,weights_file):
        ...
        self.ckpt = tf.train.get_checkpoint_state(self.weights_file)
    ...
    def draw_result(self,img,result):
        for i in range(len(result)):
            ...
            w = int(result[i][3]/2)
            h = int(result[i][4]/2)
            print('    class : ' + result[i][0] + ' , [x,y,w,h]=[' + str(x) + ',' + str(y) + ',' + str(
                int(result[i][3])) + ',' + str(int(result[i][4])) + '], Confidence = ' + str(result[i][5]))
    ...
    def interpret_output(self, output):
        probs = np.zeros((self.cell_size, self.cell_size,
                          self.boxes_per_cell, self.num_classes))
        class_probs = np.reshape(output[0:self.boundary1], (self.cell_size, self.cell_size, self.num_classes))
...
def main():
    ...
    yolo = YOLONet()
    weight_file = os.path.join(cfg.log_path)
